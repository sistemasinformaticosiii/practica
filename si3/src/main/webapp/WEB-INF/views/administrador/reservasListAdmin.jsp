	...
	<tiles:putAttribute name="body">
		...
		<div class="center_content">
			<div class="center_title_bar">
				<spring:message code="listareservas.admin.titulo"/>: <b> ${titulo}</b>
			</div>
				  ...
				  <c:when test="${not empty listaResultados}">
					...
					<c:forEach var="reserva" items="${listaResultados}">
						<tr>
							...
							<td><a href="<c:url value="/cliente/deletereserva?id=${reserva.key.reservationId}"/>"><spring:message code="listareservas.borrar"/></a></td>
						</tr>
					</c:forEach>				  
				  </c:when>
				  <c:otherwise>
				  	<b><spring:message code="listareservas.admin.mensajesinreservas"/></b>
				  </c:otherwise>
				</c:choose>
			</table>
